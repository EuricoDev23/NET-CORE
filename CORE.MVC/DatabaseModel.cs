...
namespace CORE.MVC
{
    ...
    internal sealed class DatabaseModel
    {
        ...
        private static volatile DatabaseModel DatabaseBody_ = null;
        ...
        public Dictionary<string, IEnumerable<string>> Databases { get {

                Dictionary<string, IEnumerable<string>> list = new Dictionary<string, IEnumerable<string>>();

                list.Add(Constants.Database, Tables.Where(i => i.Key.GetSourceAttribute().Database == Constants.Database && i.Key.GetSourceAttribute().Schema.Contains("dbo") == false).GroupBy(i => i.Key.GetSourceAttribute().Schema).Select(i => i.Key));
